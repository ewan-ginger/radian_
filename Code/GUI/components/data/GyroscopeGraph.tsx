...
export function GyroscopeGraph({ 
  ...
  const formatTimestamp = (timestamp: number) => {
    ...
    if (isNaN(numericTimestamp)) return '0.00';
  ...
  const formatTooltipTimestamp = (timestamp: number) => {
    const numericTimestamp = Number(timestamp);
    if (isNaN(numericTimestamp)) return 'Time: 0.00s';
            ...
            <ResponsiveContainer width="100%" height="100%">
              ...
              >
                ...
                <YAxis 
                  ...
                  label={{ value: 'Gyroscope (rad/s)', angle: -90, position: 'insideLeft', offset: -15, dy: 50 }}
                />
                <Tooltip 
                  formatter={(value, name) => [`${value.toFixed(2)} rad/s`, name]}
